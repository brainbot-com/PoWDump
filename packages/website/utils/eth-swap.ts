...
export async function commit(
    ...
    console.log('coooontract', config.ETH_POS_CONTRACT_ADDRESS, abis.abi, signer, params.claimPeriodInSec,
        params.hashedSecret,
...
export async function addRecipient(params: {
    ...
    const ethSwapContract = new Contract(config.ETH_POS_CONTRACT_ADDRESS, abis.abi, signer);
    const txResponse: TransactionResponse = await ethSwapContract.addRecipient(
        hashedSecret,
        recipient
    );
